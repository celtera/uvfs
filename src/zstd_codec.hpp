...
struct zstd_compressor
{
  ...
  [[nodiscard]] auto compress(
      char* dst, int64_t dst_capacity, const char* src, int64_t src_size,
      int level, const ZSTD_CDict* dict) const -> int64_t
  {
    size_t n = 0;
    if (dict)
      n = ZSTD_compress_usingCDict(
          ctx.get(), dst, static_cast<size_t>(dst_capacity), src,
          static_cast<size_t>(src_size), dict);
    else
      n = ZSTD_compressCCtx(
          ctx.get(), dst, static_cast<size_t>(dst_capacity), src,
          static_cast<size_t>(src_size), level);
    if (ZSTD_isError(n))
      return 0;
...
struct zstd_decompressor
{
  ...
  void decompress(
      char* dst, int64_t dst_capacity, const char* src, int64_t src_size,
      const ZSTD_DDict* dict, std::string_view what) const
  {
    size_t n = 0;
    if (dict)
    ...
    if (static_cast<int64_t>(n) != dst_capacity)
      throw std::runtime_error(
          "uvfs: " + std::string{what} + " decompressed to "
          + std::to_string(n) + " bytes but the index says "
